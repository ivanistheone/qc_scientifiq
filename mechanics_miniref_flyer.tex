...
\author{Excerpt from the MATH and PHYSICS Minireference by Ivan Savov} 
...
The key concept which we want to obtain in any physics problem is the 
\emph{equation of motion} $x(t)$ of an object, which describes its position as a function of time.
%
...
Newton's second law $F=ma$ states that  the amount of \emph{acceleration} produced by a force is inversely 
proportional to the mass of the object. Once you have the acceleration, you can compute $x(t)$ using some simple
calculus steps, which we will discuss shortly.
For now, though, we want to focus on the causes of motion: the forces.
There are many kinds of forces that could be acting on an object: the weight of an object $\vec{W}$ is a type force, 
...
The same principles of cause and effect can be also be applied to the study of circular motion.


We saw that Newton's second law and equations like 
Momentum 
